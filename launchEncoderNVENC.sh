...
ffmpeg \
    ...
    -i "$device" \
    -filter_complex \
    "[0:v]yadif=0:-1:1,fps=30,split=6[1][2][3][4][5][6]; \
    [1]hwupload_cuda,scale_npp=-1:288:interp_algo=lanczos,hwdownload[1out]; \
    [2]hwupload_cuda,scale_npp=-1:360:interp_algo=lanczos,hwdownload[2out]; \
    [3]hwupload_cuda,scale_npp=-1:432:interp_algo=lanczos,hwdownload[3out]; \
    [4]hwupload_cuda,scale_npp=-1:540:interp_algo=lanczos,hwdownload[4out]; \
...
ffmpeg \
    ...
    -i "$device" \
    -filter_complex \
    "[0:v]yadif=0:-1:1,split=6[1][2][3][4][5][6]; \
    [1]hwupload_cuda,scale_npp=-1:288:interp_algo=lanczos,hwdownload[1out]; \
    [2]hwupload_cuda,scale_npp=-1:360:interp_algo=lanczos,hwdownload[2out]; \
    [3]hwupload_cuda,scale_npp=-1:432:interp_algo=lanczos,hwdownload[3out]; \
    [4]hwupload_cuda,scale_npp=-1:540:interp_algo=lanczos,hwdownload[4out]; \
...
curl -X PUT --upload-file /tmp/${vid}.html http://${1}:8080/ldash/${vid}/index.html -H "Content-Type: text/html; charset=utf-8" >/dev/null 2>~/streamline/logs/curlIndex.log &
curl -X PUT --upload-file /tmp/${vid}.html http://${1}:8080/ldash/${vid}/hls.html -H "Content-Type: text/html; charset=utf-8" >/dev/null 2>~/streamline/logs/curlHLS.log &
curl -X PUT --upload-file /tmp/${vid}.dash.html http://${1}:8080/ldash/${vid}/dash.html -H "Content-Type: text/html; charset=utf-8" >/dev/null 2>~/streamline/logs/curlDASH.log &
curl -X PUT --upload-file /tmp/${vid}.myapp.js http://${1}:8080/ldash/${vid}/myapp.js -H "Content-Type: text/html; charset=utf-8" >/dev/null 2>~/streamline/logs/curlJS.log &
curl -X PUT --upload-file /tmp/${vid}.ldash.html http://${1}:8080/ldash/${vid}/ldash.html -H "Content-Type: text/html; charset=utf-8" >/dev/null 2>~/streamline/logs/curlLDASH.log &
...
echo "Currently streaming Low Latency DASH to dash.js http://${2}:8080/ldashplay/${vid}/manifest.mpd"
